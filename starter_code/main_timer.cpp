...
bool respawn = false;
bool go_loop = true;
bool create_level = false;                          //when a level changes, new level must be created
...
vector<Bubble> create_bubbles()
{
    ...
    bubbles.push_back(Bubble(WINDOW_X/2.0, BUBBLE_START_Y, BUBBLE_DEFAULT_RADIUS, -BUBBLE_DEFAULT_VX, BUBBLE_DEFAULT_VY, COLOR(0,0,255)));
    bubbles.push_back(Bubble(WINDOW_X/4.0, BUBBLE_START_Y - 20 , BUBBLE_DEFAULT_RADIUS, 2 *BUBBLE_DEFAULT_VX, BUBBLE_DEFAULT_VY + 0.2*BUBBLE_DEFAULT_VY, COLOR(0,0,255)));
    bubbles.push_back(Bubble(WINDOW_X/8.0, BUBBLE_START_Y, BUBBLE_DEFAULT_RADIUS, BUBBLE_DEFAULT_VX, BUBBLE_DEFAULT_VY + 0.2*BUBBLE_DEFAULT_VY, COLOR(0,0,255)));
    if(LEVEL==3) {
...
int main()
{
    ...
    while(go_loop==true)
    {
        ...
        //LIFE_str = to_string(LIFE);
        if(counter<(0.10))  {go_loop = false;}           //Game over if timer counts to zero
        Text Timer(LEFT_MARGIN+ WINDOW_X - 150, BOTTOM_MARGIN, counter);
        Text Life_value(LEFT_MARGIN+ WINDOW_X - 200, BOTTOM_MARGIN, LIFE);
        ...
        if(LEVEL<=3)collision_bubble_shooter(bubbles,shooter);
