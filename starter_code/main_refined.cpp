    ...
    while(go_loop==true)
    {
        ...
        if (pendingEvent && !respawn)
        {
            ...
            //Pause Game
                ...
                if(0.6-spam_time < 0.04) {          //Policy : upto 3 bullets can be fired in a counter interval of 0.6
                spam_time = 0;                      //set spam time counter to 0 once it reaches 0.6
                spam_bullets =0;
                }

                if(spam_bullets<=3) bullets.push_back(shooter.shoot());             //allow shooting only if upto 5 bullets have been fired upto time of spamming <=1;
                spam_bullets +=1;
                }
