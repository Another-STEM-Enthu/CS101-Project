...
void checkspam(vector<Bubble> &bubbles, vector<Bullet> &bullets){
        //cout<<bullet_time<<" no:"<<bullet_spam<<endl;
        if(5.0 - bullet_time > 0.04){                       //POLICY: max 15 bullets in an interval of 5
            if(bullet_spam>15){

                endFrame();
                Text Spam(256,276,"Spammers are cowards");
                wait(1);
                LEVEL--;
                level_change(bubbles,bullets);
                //reset spam variables once a spam is detected
                bullet_spam = 0;
                bullet_time = 0;
            }
        ...
        else{       //reset spam variables once an interval of 5 is over
            bullet_time = 0;
            bullet_spam = 0;
        }
}
...
int main()
{
    ...
    while(go_loop==true)
    {
        ...
        checkspam(bubbles,bullets);             //to check if bullets have been spammed
